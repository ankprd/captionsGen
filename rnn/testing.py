...
def litEtTraiteTsLesExemples():
	#one line = imgId#dimImgs numbers separated by ' '
        monFichier = open("ImATester.txt", "r")
	resImgs = []
	idImgs = []
...
with tf.device('/gpu:0'):    
	...
	def dynamicRNN(x, imgs, seqlen, weights, biases):
		...
		#we multiply by W and add Bias at every time step
		...
		return newOutput # de dim nbExs * nbSteps - 1 * dimMot
...
with tf.Session(config = configu) as sess:
    sess.run(init)
    saver.restore(sess, "../model/rapTr25000nH2024Ite1000000.ckpt")
